...
abstract class Model {
    ...
    public function edit() {
        ...
        $obj = new $cls;
        self::update($this->columns, "{$obj->primary_key} = ?", [$this->columns[$id]]);
    }
    ...
    public static function having($groubBy = [], $condition = '', $placeholderVals = [], $limit = 0) {
        ...
        return self::select(false, null, null, $placeholderVals, (count($groubBy)) ? implode(",", $groubBy) : null, $condition, null, $limit);
    }
    ...
    public static function first($condition = '', $placeholderVals = []) {
        $data = self::where($condition, $placeholderVals, 1);
        if (empty($data)) {
            return null;
        }
        return $data[0];
