...
class DB {
    ...
    public static function getInstance($className = '') {
        if (!isset(static::$_instance)) {
            static::$_instance = new DB($className);
        }
        static::$_instance->setClassName($className);
     ...
     */
    protected function action($action, $table, $columns = [], $condition = '', $placeholderVals = []) {
        $sql = "";
        switch ($action) {
            case 'INSERT':
    ...
    public function buildDB() {
        ...
        $files = _arrayMergeMixed(Config::app('db>dbfile'));
        $dbrestore = Config::app('mysql>corecave>dbrestore');
        $path = _path("resources.databases");
